        ...
        <div className="md:max-w-[70%] m-auto">
          <div className="flex justify-center mt-10">
            <div className="w-full h-1/2 object-cover overflow-hidden -z-10">
              <Image className="max-h-[60%]" src={company.bannerPicture} alt="company banner picture" fill={true} priority />
            </div>
          </div>
          ...
          <div>
            {allMessages.map(function (message, index) {
              return (
                <div key={message.id || index} className={`${message.sender.id === userId ? 'text-right' : 'text-left'} mb-2`}>
                  {message.sender.id === userId ? (
                    <div className="bg-gray-200 p-3 rounded-lg">
                      <p>
                        <strong>Tú: </strong>
                        {!message.sender.fullName ? `${message.sender.firstName} ${message.sender.lastName}` : message.sender.fullName}
                      </p>
                      <p>
          ...
          <button className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded" onClick={() => setShowChatBox(!showChatBox)}>
            Open Chat
          </button>
          {showChatBox && (
            <div className="bg-gray-200 p-4 rounded mt-4">
              <div className="bg-gray-200 p-4 rounded">
                <form className="flex">
                  ...
                  />
                  <button type="sumit" className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded" onClick={sendMessage}>
                    Send
                  </button>
                </form>
              </div>
